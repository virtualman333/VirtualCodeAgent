...
`renderMarkdown` 现在把表格**整块**交给 `renderMarkdownTable(lines)` 处理（列宽要看完所有行才知道，一行一行过算不出来），规则：
...
| 对齐 | 支持 `:--` / `:-:` / `--:` 三种，缺省左对齐 |
| 不猜 | 分隔行与表头格数不一致、或不是合法表格时返回 `null`，调用方原样输出 |
| 输出仍是 Markdown | 仍然是竖线 + 减号，只是补齐了空格 —— 从终端复制出去粘回 Markdown 文件依然是一张表（换成边框绘制字符好看，但粘出去只剩花纹） |
